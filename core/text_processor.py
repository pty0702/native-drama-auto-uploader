...
def _make_client(api_key, api_base_url):
    if not api_base_url:
...
def generate_summary_and_name(txt_path, api_key, text_model="doubao-seed-2-0-lite-260428", api_base_url="", log_cb=None):
    ...
    original_text = _read_txt(txt_path)
    ...
    # 清除 Windows 非法文件名字符，防止 os.rename 崩溃
    import re as _re
    sub_name = _re.sub(r'[\\/:*?"<>|\n\r\t]', '', sub_name).strip()
    sub_name = sub_name[:50] if sub_name else "短剧"
    log(f"新副名：{sub_name}")
...
def save_results(output_dir, summary, sub_name):
    ...
    new_summary_path = os.path.join(new_dir, "简介.txt")
    new_subname_path = os.path.join(new_dir, "副名.txt")

    return new_dir, new_summary_path, new_subname_path
